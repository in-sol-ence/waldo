...
class ArmPositionService(Node):
    ...
    def _load_limits(self) -> Dict[int, Tuple[float, float]]:
        path = Path(self.limit_file)
        if not path.is_absolute():
            path = Path(__file__).resolve().parent / path

        with path.open("r", encoding="utf-8") as handle:
            raw_limits = json.load(handle)

        if isinstance(raw_limits, list):
            entries = ((item["id"], item) for item in raw_limits)
        elif isinstance(raw_limits, dict):
            entries = raw_limits.items()
        else:
            raise ValueError("servoLimits.json must contain a JSON object or list")

        limits: Dict[int, Tuple[float, float]] = {}
        for raw_id, raw_limit in entries:
            servo_id = int(raw_id)
            left = float(raw_limit["limit_left"])
            right = float(raw_limit["limit_right"])
            limits[servo_id] = (min(left, right), max(left, right))
    ...
    def _validate_limits_cover_servos(self) -> None:
        ...
        if missing:
            raise ValueError(f"servoLimits.json is missing limits for servo IDs: {missing}")
