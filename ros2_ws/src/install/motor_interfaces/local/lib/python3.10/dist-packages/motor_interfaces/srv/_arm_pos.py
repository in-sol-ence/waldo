...
class ArmPos_Request(metaclass=Metaclass_ArmPos_Request):
    ...
    def __init__(self, **kwargs):
        ...
        if 'positions' not in kwargs:
            self.positions = numpy.zeros(6, dtype=numpy.int8)
        else:
            self.positions = numpy.array(kwargs.get('positions'), dtype=numpy.int8)
    ...
    def positions(self, value):
        if isinstance(value, numpy.ndarray):
            assert value.dtype == numpy.int8, \
                "The 'positions' numpy.ndarray() must have the dtype of 'numpy.int8'"
            assert value.size == 6, \
        ...
        if __debug__:
            ...
            assert \
                ...
                "The 'positions' field must be a set or sequence with length 6 and each value of type 'int' and each integer in [-128, 127]"
        self._positions = numpy.array(value, dtype=numpy.int8)
