...
class WheelVel_Request(metaclass=Metaclass_WheelVel_Request):
    ...
    def __init__(self, **kwargs):
        ...
        if 'vel' not in kwargs:
            self.vel = numpy.zeros(3, dtype=numpy.int16)
        else:
            self.vel = numpy.array(kwargs.get('vel'), dtype=numpy.int16)
    ...
    def vel(self, value):
        if isinstance(value, numpy.ndarray):
            assert value.dtype == numpy.int16, \
                "The 'vel' numpy.ndarray() must have the dtype of 'numpy.int16'"
            assert value.size == 3, \
        ...
        if __debug__:
            ...
            assert \
                ...
                "The 'vel' field must be a set or sequence with length 3 and each value of type 'int' and each integer in [-32768, 32767]"
        self._vel = numpy.array(value, dtype=numpy.int16)
